...
void CheapStepper::powerOff(){
    if (!greenMode) {
	greenMode = true;
	seq(-1); // power off
    }
}
void CheapStepper::powerOn(){
    if (greenMode) {
	seq(); // power on last seq
	greenMode = false;
    }
}
...
void CheapStepper::run(){
	...
	if (micros() - lastStepTime >= delay) { // if time for step
		if (stepsLeft > 0) { // clockwise
			...
			stepsLeft++;
			lastDoStepTime = micros();
		} else if (micros() - lastDoStepTime >= 60 * 1000000) {
			powerOff();
		} 
		lastStepTime = micros();
	}
